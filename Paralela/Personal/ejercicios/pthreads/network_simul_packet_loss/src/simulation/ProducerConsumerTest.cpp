...
int ProducerConsumerTest::start(int argc, char* argv[]) {
  ...
  // Create assembler
  this->assembler = new AssemblerTest(this->consumerDelay, this->packageLost, this->consumerCount);
  // Create assembler queue
...
int ProducerConsumerTest::analyzeArguments(int argc, char* argv[]) {
  ...
  this->packageLost = std::atoi(argv[index++]);

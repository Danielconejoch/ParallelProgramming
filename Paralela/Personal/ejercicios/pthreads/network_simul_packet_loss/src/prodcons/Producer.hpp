...
class Producer : public virtual Thread {
 ...
 public:
  /// Constructor
  explicit Producer(Queue<DataType>* sharedQueue = nullptr)
    : producingQueue(sharedQueue) {
  }
  ...
  inline void setProducingQueue(Queue<DataType>* sharedQueue) {
    this->producingQueue = sharedQueue;
  }
